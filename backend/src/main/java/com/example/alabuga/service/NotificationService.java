...
public class NotificationService {
    ...
    public void createRankPromotionNotification(User user, Rank oldRank, Rank newRank) {
        String title = "СИСТЕМНЫЙ ЖУРНАЛ: ПОВЫШЕНИЕ СТАТУСА";
    ...
    private String generateMissionCompletedContent(String missionName, Integer experienceReward, Integer energyReward) {
        return String.format("""
            ...
            Продолжайте в том же духе, оперативник.
    ...
    private String generateArtifactAcquiredContent(String artifactName, String artifactRarity) {
        return String.format("""
            // ОПОВЕЩЕНИЕ СЛУЖБЫ АРТЕФАКТОВ //
            Обнаружен новый артефакт: «%s»
            Редкость: %s
            
            Артефакт добавлен в ваш инвентарь. Используйте его мудро.
    ...
    private String generateShopPurchaseContent(String itemName, Integer price) {
        return String.format("""
            // ОПОВЕЩЕНИЕ НЕКСУСА //
            Покупка подтверждена: «%s»
            Стоимость: %d Энергонов
            
            Товар добавлен в ваш инвентарь. Удачных операций!
            """, itemName, price);
    }
    ...
    private String createArtifactMetadata(String artifactName, String artifactRarity) {
        try {
            ...
            return "{\"artifactName\": \"" + artifactName + "\", \"rarity\": \"" + artifactRarity + "\"}";
        }
    }
    ...
    private record RankPromotionMetadata(Integer oldRankLevel, String oldRankName, Integer newRankLevel, String newRankName) {}
