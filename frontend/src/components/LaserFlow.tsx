...
// inlined base styles via component style prop; dedicated css file removed
...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const setSizeNow = () => {
      ...
      rectRef.current = canvasRect as DOMRect;
    };
    ...
    canvas2.addEventListener('pointermove', onMove as any, { passive: true });
    canvas2.addEventListener('pointerdown', onMove as any, { passive: true });
    canvas2.addEventListener('pointerenter', onMove as any, { passive: true });
    canvas2.addEventListener('pointerleave', onLeave as any, { passive: true });
    ...
    canvas2.addEventListener('webglcontextlost', onCtxLost, false);
    canvas2.addEventListener('webglcontextrestored', onCtxRestored, false);
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      else if (avgFps > upperThresh && currentDprRef.current < base) { next = clamp(currentDprRef.current * 1.05, dprFloor, base); }
    ...
    const animate = () => {
      ...
      if (!hasFadedRef.current) {
        const fadeDur = 1.0;
        let fade = (uniforms.uFade.value as number) || 0;
      ...
      const prev = new THREE.Vector2((uniforms.iMouse.value as THREE.Vector4).x, (uniforms.iMouse.value as THREE.Vector4).y);
      prev.lerp(new THREE.Vector2(mouseTarget.x, mouseTarget.y), alpha);
      (uniforms.iMouse.value as THREE.Vector4).set(prev.x, prev.y, 0, 0);
    ...
    return () => {
      ...
      canvas2.removeEventListener('pointermove', onMove as any);
      canvas2.removeEventListener('pointerdown', onMove as any);
      canvas2.removeEventListener('pointerenter', onMove as any);
      canvas2.removeEventListener('pointerleave', onLeave as any);
      canvas2.removeEventListener('webglcontextlost', onCtxLost as any);
      canvas2.removeEventListener('webglcontextrestored', onCtxRestored as any);
      geometry.dispose();
      (material as any).dispose();
      renderer.dispose();
      if (mount.contains(canvas2)) mount.removeChild(canvas2);
    };
  }, [dpr]);
  ...
  const baseStyle: React.CSSProperties = { width: '100%', height: '160px', position: 'relative', pointerEvents: 'none' };
  return <div ref={mountRef} className={className} style={{ ...baseStyle, ...style }} />;
};
